...
# First, detect CUDA version if available
CUDA_VERSION=""
CUDA_DOCKER_TAG="12.9.0-base-ubuntu24.04"  # Default fallback
...
if command -v nvidia-smi &> /dev/null; then
    ...
    if [ ! -z "$CUDA_VERSION" ]; then
        ...
        # Map CUDA version to appropriate Docker image
        case $CUDA_VERSION in
            "12.8")
                CUDA_DOCKER_TAG="12.8.0-base-ubuntu24.04"
                ;;
            "12.7")
                CUDA_DOCKER_TAG="12.7.0-base-ubuntu24.04"
                ;;
            "12.6")
                CUDA_DOCKER_TAG="12.6.0-base-ubuntu24.04"
                ;;
            "12.5")
                CUDA_DOCKER_TAG="12.5.0-base-ubuntu24.04"
                ;;
            "12.4")
                CUDA_DOCKER_TAG="12.4.0-base-ubuntu24.04"
                ;;
            "12.3")
                CUDA_DOCKER_TAG="12.3.0-base-ubuntu24.04"
                ;;
            "12.2")
                CUDA_DOCKER_TAG="12.2.0-base-ubuntu24.04"
                ;;
            "12.1")
                CUDA_DOCKER_TAG="12.1.0-base-ubuntu24.04"
                ;;
            "12.0")
                CUDA_DOCKER_TAG="12.0.0-base-ubuntu24.04"
                ;;
            "11.8")
                CUDA_DOCKER_TAG="11.8.0-base-ubuntu22.04"
                ;;
            "11.7")
                CUDA_DOCKER_TAG="11.7.0-base-ubuntu22.04"
                ;;
            *)
                echo "   ⚠️  CUDA version $CUDA_VERSION not explicitly supported, using default $CUDA_DOCKER_TAG"
                ;;
        esac
        echo "   🐳 Using CUDA Docker image: nvidia/cuda:$CUDA_DOCKER_TAG"
    else
        echo "   ⚠️  Could not detect CUDA version, using default: $CUDA_DOCKER_TAG"
    fi
else
    echo "   ⚠️  nvidia-smi not found, using default CUDA image: $CUDA_DOCKER_TAG"
fi

# Now test GPU support with the appropriate CUDA image
if ! docker run --rm --gpus all nvidia/cuda:$CUDA_DOCKER_TAG nvidia-smi &>/dev/null; then
    ...
    echo "✅ GPU support detected! Getting GPU information..."
    docker run --rm --gpus all nvidia/cuda:$CUDA_DOCKER_TAG nvidia-smi --query-gpu=name,memory.total,driver_version --format=csv,noheader,nounits | while read line; do
        echo "   🚀 $line"
    done
    echo "   💪 GPU acceleration enabled for Ollama embeddings"
    GPU_SUPPORT=true
fi
...
# Step 2.5: Clean up any existing containers
echo "🧹 Cleaning up any existing LightRAG containers..."
...
if [ "$GPU_SUPPORT" = true ]; then
    echo "   - Embeddings: Local Ollama (bge-m3:latest) with GPU acceleration"
else
    echo "   - Embeddings: Local Ollama (bge-m3:latest) - CPU only"
fi
